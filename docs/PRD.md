...
- [x] merchant Midtrans: kunci sandbox dan production diterima 22 Sep, alur bayar lolos uji sandbox 23 Sep (laporan ke Andrew via Telegram); tersisa konfirmasi dokumen badan usaha dan Notification URL di dashboard production
...
- Go-live: (1) di dashboard production (dashboard.midtrans.com) isi Settings > Configuration > Payment Notification URL = `https://kuwera5k.vercel.app/api/payments/webhook` (di sandbox sudah diisi 23 Sep); (2) di Vercel pindahkan nilai `MIDTRANS_PROD_SERVER_KEY` ke `MIDTRANS_SERVER_KEY` dan `MIDTRANS_PROD_CLIENT_KEY` ke `NEXT_PUBLIC_MIDTRANS_CLIENT_KEY`, set `MIDTRANS_IS_PRODUCTION=true`, deploy ulang; (3) hapus order uji sebelum pendaftaran dibuka (order berlabel `mock` di tabel payments dan order sandbox bernama "Uji ..."); (4) pastikan channel QRIS, VA, e-wallet, dan kartu aktif di merchant production (merchant sandbox hanya punya GoPay QRIS, 6 VA, kartu, ShopeePay QRIS).
