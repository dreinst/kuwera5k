...
| Kuota total | 1.500 peserta | Target panitia (Donny, 24 Sep 2026) |
| Jendela pendaftaran | Ditutup begitu kuota 1.500 penuh; kalau belum penuh, ditutup H-1 minggu, Sabtu 17 Oktober 2026 pukul 23.59 WIB, karena ukuran jersey harus dikirim ke vendor untuk dicetak (sama dengan jendela jual kategori). Section Jadwal lomba di beranda menampilkan tanggal ini | Donny, 24 Sep 2026 (tanggal 17 Okt bisa digeser) |
| Biaya pendaftaran | Rp125.000 per peserta, satu kategori, tanpa early bird | Tetap (Donny, 22 Sep 2026) |
| Pengambilan race pack | Kamis dan Jumat, 22 sampai 23 Oktober 2026 (H-2 dan H-1), jam menyusul | Donny, 24 Sep 2026; jam belum ada |
...
- Database PostgreSQL 16 di VPS D'Pro, dikelola lewat Coolify. Detail akses (host, port, nama resource, aturan firewall) sengaja tidak ditulis di repo publik ini; ada di catatan internal tim.
...
- Koneksi ke database (sejak 24 September 2026): TLS terverifikasi dengan CA server (`DB_SSL_CA`, nama sertifikat lewat `DB_SSL_SERVERNAME` saat terhubung lewat IP), koneksi tanpa TLS ditolak server. Aplikasi memakai akun `kuwera_app` (hanya baca/tulis data, tidak bisa mengubah skema), perubahan skema memakai `kuwera_owner` lewat `DATABASE_URL_MIGRASI`, superuser tidak bisa login lewat jaringan dan kata sandinya sudah diganti. Pool koneksi per instance dibatasi 5. `DATABASE_URL` tidak boleh memuat `sslmode` (parameter URL mengalahkan pengaturan TLS di kode).
...
- Hero: pil "Fun Run 5K Malang 2026", judul terpusat "KUWERA" (garis tepi) "FUN RUN" (isi kuning) "5K" (garis tepi), deskripsi, lalu "Diselenggarakan oleh" + logo D'Pro (24 Sep diganti hitung mundur). Kartu tiga kolom bertingkat: kiri Save the date (tanggal besar, tombol Tambah ke kalender Google, selesai = start + batas waktu 60 menit) dan biaya pendaftaran; tengah jumlah pendaftar + tombol Daftar dan kartu medali kuning ("Medali finisher menunggu kamu!"); kanan kartu jersey (menggantikan kartu rute di konsep, karena kartu rute sudah diminta dihapus dari hero).
...
Audit keamanan 24 September 2026 (kode, dependensi, konfigurasi, VPS). Sudah diperbaiki: header keamanan (CSP yang sudah diuji dengan popup Snap sandbox dan Turnstile, X-Frame-Options, nosniff, Referrer-Policy, Permissions-Policy, tanpa x-powered-by); pembatas laju per IP di Postgres untuk buat order (20/10 menit), promo (20/10 menit), status order (300/10 menit), Snap (30/10 menit), login admin (10/15 menit), halaman /bayar dan /tiket (120/10 menit); kunci akun admin atomik dan pesan gagal yang sama untuk semua kasus; keluar admin mematikan semua sesi akun itu; email disamarkan di /bayar dan /tiket; alamat kembali Snap memakai alamat situs; panitia melihat NIK tersamar dan tanpa alamat; pesan error form hanya untuk kolom yang sudah disentuh (sebelumnya pesan kolom lain hilang saat fokus pindah dan klik Lanjut pertama meleset); backup harian database ke lokal dan NAS. Lanjutan 24 September 2026: TLS database, akun database hak minimum, dan penggantian kata sandi superuser sudah dikerjakan (lihat bagian koneksi database di atas); VPS sudah diperbarui (paket OS, Docker, containerd, Tailscale, kernel) dan direboot. Cloudflare Turnstile aktif sejak 24 September 2026 dengan widget sendiri bernama "kuwera" (mode Managed, hostname kuwera5k.vercel.app; subdomain dpro.events ditambahkan saat pindah ke VPS), di langkah pembayaran form daftar dan di halaman masuk admin. Masih menunggu: aturan firewall Vercel. Rencana berikutnya: aplikasi pindah ke VPS sebagai subdomain dpro.events, lalu port database publik ditutup.

Revisi beranda dan form 24 September 2026 (permintaan Donny):
- Section rute: kolom keterangan checkpoint dihapus, tinggal kartu urutan jalan di kanan peta yang tingginya sama dengan kartu peta; legenda pindah ke bawah peta. Data `route.checkpoints` tetap ada karena posisi water station di peta dihitung darinya.
- Hero: baris "Diselenggarakan oleh" + logo D'Pro diganti hitung mundur ke start (hari, jam, menit, detik; jam dibaca dari browser). Logo D'Pro tetap di footer. Revisi 25 Sep: hitung mundur sebaris dengan judul KUWERA FUN RUN 5K di layar lebar (di HP turun ke bawah judul), di bawahnya tagline "Kejar garis finish, bawa pulang medalinya!" lalu "Diselenggarakan oleh Keuangan Angkatan Darat Malang." Pil "Fun Run 5K Malang 2026" di atas judul dihapus.
- Section baru Jadwal lomba (setelah pita tanggal, meniru "Race Schedule" malangrun10k.com): periode pendaftaran, pengambilan race pack, hari lomba, lalu tombol Daftar sekarang. Revisi 25 Sep (Donny: kalimatnya terkesan kurang siap): "jam menyusul" dan kalimat soal vendor jersey dihapus dari kartu dan FAQ; jam race pack tetap tertulis menyusul di e-ticket peserta.
- Data terstruktur: `organizer` berisi Keuangan Angkatan Darat Malang dan D'Production Event Organizer.
- Form: kota/kabupaten jadi pilihan yang isinya mengikuti provinsi (514 kabupaten/kota dari Kepmendagri No. 300.2.2-2138 Tahun 2025 lewat data cahyadsn/wilayah, `src/lib/wilayah.ts`), terkunci sampai provinsi dipilih dan kosong lagi kalau provinsi diganti. Server menolak kota yang bukan bagian dari provinsinya.
- Captcha tetap Cloudflare Turnstile (reCAPTCHA Google tidak dipakai).
